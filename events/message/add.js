...
module.exports = async event => {
  ...
  const attachments = []
  try {
    await user.addChannel(channel.info.id)
    text = Message.channel_add_success
    attachments.push({ text: `#${ch.name}` })
  } catch (e) {
    /* istanbul ignore else / outer error */
    if (e.message === 'channel_duplicate') {
      body = 'channel_duplicate'
      text = Message.channel_duplicate
      attachments.push({ text: `#${ch.name}` })
    } else {
      body = 'channel_add_error'
      text = Message.channel_add_error
    }
  }
  // Slackに投稿
  await slack.bot({
